...
def index():
    ...
    return render_template('index.html', form=form, posts=post_list,
...
def user(username):
    if username is not None:
        ...
        page = request.args.get('page', 1, type=int)
        pagination = curr_user.posts.order_by(Post.timestamp.desc()).paginate(page, per_page=current_app.config['FLASKY_POSTS_PER_PAGE'],
            error_out=False)
        post_list = pagination.items
        return render_template('user.html', user=curr_user, posts=post_list,
                           pagination=pagination)


@main.route('/my-account/edit-profile', methods=['GET', 'POST'])
@login_required
def edit_profile():
